    ...
    init: function (callback) {
        var ready = function () {
            try {
                ...
                if (getQuerystring('jotform_pwa') === '1' || (JotForm.isAdmin && !JotForm.enterprise)) {
                    this.setJotFormUserInfo();
                }
                ...
                try {
                    ...
                    if ($$('input, select, textarea').length > 900) { //collapse matrices for long forms
                        $$('.form-matrix-table').each(function (matrixTable) {
                            ...
                            var staticMatrix = inputs.every(function (input) { return input.type === inputs[0].type });
                            inputs.each(function (input) {
                                var ids = input.id.split("_");
                                var x = ids[2];
                                var y = ids[3];
                                if (input.type == "radio" && staticMatrix) {
                                    if (input.checked) {
